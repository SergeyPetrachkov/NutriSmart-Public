# NutriSmart – Terms of Service

**Last updated:** 2026-01-20

Welcome to **NutriSmart** (“NutriSmart”, “we”, “us”, or “our”).  
These Terms of Service (“Terms”) govern your access to and use of the NutriSmart mobile application and related services (the “Service”).

By using the Service, you agree to these Terms. If you do not agree, do not use NutriSmart.

---

## 1. Description of the Service

NutriSmart is a nutrition tracking application that allows users to:

- Track meals, calories, and macronutrients
- Define fitness and nutrition goals
- Enter meals using natural language
- Use AI-powered features to estimate meal composition and macro splits

### AI Processing Overview

- The **free version** of NutriSmart uses **on-device Apple Foundation Models**
- AI processing in the free version occurs **entirely on the user’s device**
- Certain optional or advanced features may use **Google Gemini via Firebase**, which requires sending user input to third-party servers

---

## 2. No Medical or Professional Advice

NutriSmart is provided **for informational and educational purposes only**.

- NutriSmart does not provide medical, nutritional, or dietary advice
- NutriSmart is not a substitute for professional healthcare advice
- Always consult a qualified healthcare professional before making dietary changes

You use the Service at your own risk.

---

## 3. AI-Generated Content Disclaimer

Some features rely on artificial intelligence (AI).

You acknowledge that:
- AI-generated results may be inaccurate or incomplete
- Nutrition values are estimates only
- NutriSmart does not guarantee correctness of AI output

You are responsible for verifying any information before relying on it.

---

## 4. User Responsibilities

You agree to:
- Use the Service for lawful purposes only
- Not misuse or abuse AI features
- Not attempt to reverse-engineer or exploit the Service

You are solely responsible for how you use NutriSmart and its outputs.

---

## 5. Third-Party Services

Depending on the features used, NutriSmart may rely on third-party services.

- **Free version:** AI processing occurs on-device using Apple Foundation Models
- **Other features:** user input may be transmitted to Google Gemini via Firebase for processing

NutriSmart does not control how third-party providers process or store data.  
Use of such services is governed by their respective terms and privacy policies.

---

## 6. Intellectual Property

All content, software, and technology used in NutriSmart are owned by or licensed to NutriSmart and are protected by applicable laws.

You may not copy, modify, or distribute any part of the Service without permission.

---

## 7. Termination

We may suspend or terminate access to the Service at any time if:
- These Terms are violated
- The Service is discontinued
- Required by law

Upon termination, your right to use the Service ends immediately.

---

## 8. Disclaimer of Warranties

The Service is provided **“AS IS” and “AS AVAILABLE”** without warranties of any kind.

We do not guarantee uninterrupted operation or accuracy of results.

---

## 9. Limitation of Liability

To the maximum extent permitted by law, NutriSmart shall not be liable for:

- Indirect or consequential damages
- Health-related outcomes
- Errors in AI-generated data

Your sole remedy is to stop using the Service.

---

## 10. Governing Law

These Terms are governed by the laws of **[The Netherlands]**, without regard to conflict of law rules.

---

## 11. Contact

For questions about these Terms:

📧 **[smart.longevity.solutions@gmail.com]**
